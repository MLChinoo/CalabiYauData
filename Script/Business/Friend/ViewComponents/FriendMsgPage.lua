...
function FriendMsgPage:ShowMsg()
  ...
  if self.CanvasPanel_Total then
    self.CanvasPanel_Total:SetVisibility(UE4.ESlateVisibility.Collapsed)
  end
      ...
      if curMsg then
        if self:SetNtfInfo(curMsg) then
          if self.showMsgTask == nil then
            self.showMsgTask = TimerMgr:AddFrameTask(2, 0, 1, function()
              ...
              if self.CanvasPanel_Total then
                self.CanvasPanel_Total:SetVisibility(UE4.ESlateVisibility.SelfHitTestInvisible)
              end
            end)
          end
        else
...
function FriendMsgPage:Destruct()
  ...
  self:UnbindAllFromAnimationFinished(self.FriendMsn_Widget_Open)
...
function FriendMsgPage:SetNtfInfo(inMsg)
  ...
  elseif inMsg.msgType == FriendEnum.FriendMsgType.NewMail then
    if self.NewMailTipAkAudio then
      local Audio = UE4.UPMLuaAudioBlueprintLibrary
      _ = self.PlayingId and Audio.StopPlayingID(self.PlayingId)
      self.PlayingId = Audio.PostEvent(Audio.GetID(self.NewMailTipAkAudio))
    end
    WS_IconIndex = 3
  end
...
function FriendMsgPage:OnClickInputClick()
  ...
  elseif self.curMsgType == FriendEnum.FriendMsgType.NewMail then
    GameFacade:SendNotification(NotificationDefines.JumpToPageCmd, {
      target = UIPageNameDefine.KaPhonePage
    })
    GameFacade:SendNotification(NotificationDefines.NtfKaPhoneOpenMail)
  end
